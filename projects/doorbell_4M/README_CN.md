...
本项目是一个基于BK7258芯片的智能门铃门锁解决方案，支持通过WiFi传输图像数据、本地LCD屏幕显示以及实时双向音频通信。项目集成了丰富的多媒体处理能力、网络通信功能和用户界面显示能力，适用于智能门铃门锁设备的开发。相比基础doorbell工程，doorbell_4M侧重于摄像头MJPEG图传与LCD显示。
...
- 支持软件或硬件解码，系统内部自动选择
- 支持H.264视频传输，通过H.264编码器管道进行硬件编码
- 支持默认三种帧的buffer池管理
- 支持多种LCD屏幕显示，RGB屏或MCU屏，默认使用RGB屏（st7701sn）
- 支持多种音频编解码算法，默认使用G711编码
- 支持板载speaker输出和UAC音频输入输出，不支持板载mic输入
- 支持多种传输协议，实时传输音视频数据
...
编译生成的烧录bin文件路径：``projects/doorbell_4M/build/bk7258/app/package/all-app.bin``
...
7. 配网完成之后会自动打开UVC摄像头，并开启网络图传，传输格式为H.264，图像分辨率为864x480
8. 打开其他外设，可以在IOT应用上进行控制。

## 4 Doorbell 4M视频流方案

本项目支持UVC摄像头方案。doorbell_4M工程不支持DVP摄像头方案。

### 4.1 UVC摄像头 + MJPEG + LCD显示 + H264网络传输
...
doorbell_4M工程不支持DVP摄像头方案。本文档中的视频采集、本地显示和网络传输说明均基于UVC摄像头路径。
...
**UVC方案**：
  ...
  - ✅ H264 Frame Queue：H264编码器输出
  - ❌ DVP摄像头路径：doorbell_4M不支持
...
**UVC方案性能**：
  ...
  - **H264硬件编码**：864x480@30fps，延迟<50ms
 ...
 * 
 * @note 内存管理方式：
 *       - UVC方案：MJPEG使用队列，解码后YUV直接申请内存，H264编码器输出使用队列
 *       - DVP摄像头路径在doorbell_4M中不支持
 */
...
> **注意**：此API用于UVC路径通过编码器管道进行H264硬件编码。doorbell_4M不支持DVP摄像头路径。
 ...
 * 
 * @note 数据来源：
 *        - UVC方案：从H264 Frame Queue获取硬件编码后的H264帧
 *        - DVP摄像头路径在doorbell_4M中不支持
 * 
...
5. **内存管理说明**：
   - UVC方案（MJPEG解码 + H264硬件编码）：

     * ✅ MJPEG帧：使用MJPEG Frame Queue V2管理
     * ❌ 解码后的YUV数据：**直接内存申请**，不使用YUV Frame Queue
     * ❌ 视频处理管道的输出：直接内存申请
     * ✅ H264帧：硬件编码后使用H264 Frame Queue管理
     * 📌 处理开销：需要MJPEG解码和H264编码器管道

   - doorbell_4M不支持DVP摄像头路径。
