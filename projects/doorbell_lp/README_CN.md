...
本项目是基于BK7259芯片的智能门铃门锁 **低功耗保活解决方案**，在doorbell工程基础上增加了 **AP掉电低功耗保活功能**。
...
烧录流程参考 BK7259 对应 SDK 文档中的烧录章节。

编译生成的烧录bin文件路径：``projects/doorbell_lp/build/bk7259/doorbell_lp/package/all-app.bin``
...
doorbell_lp工程采用BK7259双核架构实现低功耗保活功能：

![Doorbell LP 架构图](../../docs/bk7259/_static/doorbell_lp.png)
...
#### 4.4.6 保活端口绑定至CP端口范围机制

该机制提供了一种将对端发往本机的保活相关报文直接交给 **CP侧协议栈** 处理的方法，CP侧在建立保活TCP连接时可以将 socket **绑定到 CP 的端口范围**，再发起 `connect()`。

**说明：**

- CP侧接收通路会根据 **目的端口** 决定上行数据是送给CP还是AP：目的端口落在 **CP 本地端口范围** （如 `LOCAL_PORT_RANGE_START` ～ `LOCAL_PORT_RANGE_END` 为0x1000～0x1010）的报文会被上传到CP协议栈。
- LwIP 默认分配的临时端口在 0xc000～0xffff，该范围不在上述 CP 端口范围内，对端回复的报文可能被送到AP侧协议栈，导致 CP 侧连接无法正常收尾。
- 将保活端口直接绑定到CP端口范围，这样保活连接使用的本地端口一定落在 CP 端口范围内，对端发回的报文目的端口也在该范围内，CIF 会将其送给 CP 侧协议栈，从而在 CP 侧完成正常建连、收包和关闭流程。
 ...
 *       2. 启动AP电源（bk_pm_module_vote_boot_ap_ctrl）
 ...
 *       2. 关闭AP电源（bk_pm_module_vote_boot_ap_ctrl）
