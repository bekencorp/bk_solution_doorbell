...
- :strong:`CP` （M52）：``bk_init()`` 初始化 Wi-Fi 协议栈、BLE Controller、射频校准（``vnd_cal``）及 CIF 等与 AP 的通信底层；随后 ``bk_start_ap_system()`` 启动 AP，并持续运行无线侧控制（Wi-Fi/BLE 共存）。
- :strong:`AP` （M55）：运行多媒体与门铃业务（``media_service``、``devices_mgmt``、``smart_lock`` 的 doorbell_core/boarding/network、BLE Host、ASR 等）。
