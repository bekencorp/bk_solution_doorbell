...
本工程是 doorbell 解决方案中的 :strong:`IPC 网络摄像头方案`，基于 BK7259 SMP 双核架构。
与 ``doorbell`` 共用 ``smart_lock`` 门铃业务栈（BLE 配网、TCP/UDP 图传、双向音频、ASR），但 :strong:`默认不启用 LCD/GPU 显示链路`，
面向无屏 IPC / 网络摄像头产品。
...
| 核心                 | 职责                                                      |
+======================+===========================================================+
| :strong:`CP` （M52） | ``bk_init()`` 与 ``bk_start_ap_system()`` （同 doorbell） |
+----------------------+-----------------------------------------------------------+
| :strong:`AP` （M55） | media_service、devices_mgmt、doorbell 业务、ASR、AT 命令  |
+----------------------+-----------------------------------------------------------+
